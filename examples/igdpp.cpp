/* IGD pretty-printing tool. This mostly just exists for demonstrating and
...
int main(int argc, char *argv[]) {
  ...
  if (argc < 3) {
    std::cerr << "Please pass in a command and an input file (in that order)"
              << std::endl;
    return 1;
  }
  ...
  if (command == "stats") {
    ...
    std::vector<size_t> sampleToMuts(
        igd.numSamples()); // Counts muts per sample
