...
int main(int argc, char *argv[]) {
  ...
  args::Positional<std::string> infile(
      parser, "input_file", "The input file (.vcf, .vcf.gz, or .igd)");
  ...
  args::ValueFlag<std::string> outputDescription(
      parser, "outputDescription",
      "The description string to include in the IGD output.", {"description"});
  args::ValueFlag<std::string> range(
      parser, "range", "Restrict to the given base-pair range (inclusive).",
      {'r', "range"});
  args::ValueFlag<std::string> frange(parser, "frange",
                                      "Restrict to variants with frequency in "
                                      "the range (inclusive, exclusive).",
                                      {'f', "frange"});
  args::Flag info(parser, "info", "Display information from the IGD header.",
                  {'i', "info"});
  args::Flag individuals(parser, "individuals",
                         "Emit the mapping from individual index to ID.",
                         {"individuals"});
  args::Flag variants(parser, "variants",
                      "Emit the mapping from variant index to ID.",
                      {"variants"});
  args::Flag stats(parser, "stats",
                   "Emit some simple statistics about the distribution of "
                   "samples, sites, and variants.",
                   {'s', "stats"});
  args::Flag alleles(parser, "alleles", "Emit allele frequencies.",
                     {'a', "alleles"});
  args::Flag noIndividualIds(
      parser, "noIndividualIds",
      "Do not emit IDs for individuals in the resulting IGD file.",
      {"no-indiv-ids"});
  args::Flag noVariantIds(
      parser, "noVariantIds",
      "Do not emit IDs for variants in the resulting IGD file.",
      {"no-var-ids"});
  args::ValueFlag<size_t> trimSamples(parser, "trimSamples",
                                      "Trim samples to this many individuals.",
                                      {"trim"});
  try {
...
#define UNSUPPORTED_FOR_VCF(parameter, parameterName)                          \
  do {                                                                         \
    if ((bool)(parameter)) {                                                   \
      std::cerr << "Parameter " << (parameterName)                             \
                << " is not supported for VCF file conversion" << std::endl;   \
      return 1;                                                                \
    }                                                                          \
  } while (0)
  ...
  const bool isVcf =
      ends_with(*infile, ".vcf") || ends_with(*infile, ".vcf.gz");
  if (isVcf) {
    if (!outfile) {
      std::cerr
          << "VCF input is only supported for conversion to IGD. Use --output."
          << std::endl;
      return 1;
    }
    ...
    vcfToIGD(*infile, *outfile, description, true, emitIndividualIds,
             emitVariantIds);
    return 0;
  }
  ...
  if (range) {
    ...
    if (endptr >= rangecEnd || *endptr != '-') {
      std::cerr << "Malformed range: " << *range << " (must be \"lower-upper\")"
                << std::endl;
      return 1;
    }
    endptr++;
    ...
    std::cout << "Restricting to base-pair range " << bpStart << " - " << bpEnd
              << " (inclusive)" << std::endl;
  }
  ...
  if (frange) {
    ...
    if (endptr >= frangecEnd || *endptr != '-') {
      std::cerr << "Malformed frange: " << *frange
                << " (must be \"lower-upper\")" << std::endl;
      return 1;
    }
    endptr++;
    ...
    std::cout << "Restricting to allele frequencies between [" << fLower << ", "
              << fUpper << ")" << std::endl;
  }
  ...
  if (trimSamples) {
    ...
    if (effectiveSampleCt > numSamples) {
      std::cerr << "--trim value is larger than the number of individuals"
                << std::endl;
      return 2;
    }
    std::cout << "Trimming to " << effectiveSampleCt << " haploid samples"
              << std::endl;
  }
  ...
  if (info) {
    ...
    std::cout << "  Phased?: " << (igd.isPhased() ? "true" : "false")
              << std::endl;
    std::cout << "  Source: " << igd.getSource() << std::endl;
    std::cout << "  Genome range: " << igd.getPosition(0) << "-"
              << igd.getPosition(igd.numVariants() - 1) << std::endl;
    std::cout << "  Has individual IDs? "
              << (igd.getIndividualIds().empty() ? "No" : "Yes") << std::endl;
    std::cout << "  Has variant IDs? "
              << (igd.getVariantIds().empty() ? "No" : "Yes") << std::endl;
  }
  if (individuals) {
...
#define CONDITION_PRINT(condition, message)                                    \
  ...
  if (outfile) {
    ...
    writer =
        std::make_shared<IGDWriter>(ploidy, numIndividuals, igd.isPhased());
  ...
  if (iterateSamples) {
    ...
    CONDITION_PRINT(alleles, "POSITION" << SEP << "REF" << SEP << "ALT" << SEP
                                        << "ALT COUNT" << SEP << "TOTAL"
                                        << std::endl);
    ...
    std::vector<size_t> sampleToMuts(
        effectiveSampleCt); // Counts muts per sample
    ...
    for (size_t i = 0; i < igd.numVariants(); i++) {
      ...
      if (pos >= bpStart && pos <= bpEnd) {
        ...
        CONDITION_PRINT(alleles, pos << SEP << ref << SEP << alt << SEP
                                     << sampleCt << SEP << effectiveSampleCt
                                     << std::endl);
        if (outfile) {
          writer->writeVariantSamples(*igdOutfile, pos, ref, alt, sampleList,
                                      isMissing);
        }
        if (stats) {
          variants++;
