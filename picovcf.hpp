...
static constexpr SampleT SAMPLE_INDEX_NOT_SET =
    std::numeric_limits<SampleT>::max();
...
static constexpr VariantT MISSING_VALUE =
    std::numeric_limits<VariantT>::max() - 1;

static constexpr size_t INTERNAL_VALUE_NOT_SET =
...
public:
  explicit ApiMisuse(char const *const message) : std::runtime_error(message) {}
};
...
#define PICOVCF_RELEASE_ASSERT(condition)                                      \
  do {                                                                         \
    if (!(condition)) {                                                        \
      std::cerr << "PICOVCF_RELEASE_ASSERT(" #condition ") failed at "         \
...
#define PICOVCF_GOOD_OR_READ_ERROR(fstream, filename)                          \
  do {                                                                         \
    if (!(fstream).good()) {                                                   \
      PICOVCF_THROW_ERROR(FileReadError,                                       \
                          "Failure to read input file " << (filename));        \
    }                                                                          \
  } while (0)
...
#define PICOVCF_GOOD_OR_MALFORMED_FILE(fstream)                                \
  do {                                                                         \
    if (!(fstream).good()) {                                                   \
      PICOVCF_THROW_ERROR(                                                     \
          MalformedFile, "Unexpected file error reading input file (position " \
                             << (fstream).tellg() << ")");                     \
    }                                                                          \
  } while (0)
  ...
  do {                                                                         \
    if (!(fstream).good()) {                                                   \
      PICOVCF_THROW_ERROR(ApiMisuse,                                           \
                          "File error when seeking to user-supplied location " \
                              << (fstream).tellg() << ")");                    \
    }                                                                          \
  } while (0)
...
// be divisible by 8).
template <typename T, size_t TO> inline T picovcf_div_ceiling(T value) {
  return (value + (TO - 1)) / TO;
}

inline void picovcf_split(const std::string &theString, const char token,
...
inline std::string picovcf_getValue(const std::string &line,
                                    const size_t start) {
  size_t position = line.find_first_of('=', start);
  if (position != std::string::npos) {
...
public:
  BufferedReader(const std::string &filename, size_t bufferedByteCt)
      : m_file(std::fopen(filename.c_str(), "rb")), m_capacity(bufferedByteCt),
        m_position(0) {
...
public:
  ZBufferedReader(const std::string &filename, size_t bufferedByteCt)
      : BufferedReader(filename, bufferedByteCt), m_compressed(bufferedByteCt),
        m_lastFileRead(0) {
...
private:
  ...
  size_t zReadBuffered() {
    ...
    m_zlibStream.avail_in =
        std::fread(m_compressed.data(), 1, m_compressed.size(), m_file);
    if (std::ferror(m_file)) {
      PICOVCF_THROW_ERROR(FileReadError,
                          "Failed to read input file @ " << m_lastFileRead);
    }
...
public:
  ...
  void next() {
    ...
    m_individualIndex++;
    m_currentPosition =
        m_currentLine.find_first_of('\t', m_currentPosition + 1);
    if (m_currentPosition != std::string::npos) {
...
private:
  IndividualIteratorGT(const std::string &currentLine,
      ...
      : m_currentLine(currentLine), m_currentPosition(currentPosition),
        m_individualIndex(0) {}
  ...
  static inline VariantT stringToVariantT(const std::string &str) {
    ...
    char *endPtr = nullptr;
    VariantT result =
        static_cast<uint32_t>(std::strtoull(str.c_str(), &endPtr, 10));
...
public:
  ...
  inline VCFVariantInfo parseToVariantInfo(bool basicInfoOnly = true) {
    VCFVariantInfo result = {
        this->getChrom(),      this->getPosition(),
        this->getID(),         this->getRefAllele(),
        this->getAltAlleles(), std::numeric_limits<double>::quiet_NaN()};
    if (!basicInfoOnly) {
  ...
  size_t getPosition() const {
    const size_t posSize =
        m_nonGTPositions[POS_POS_END] - m_nonGTPositions[POS_CHROM_END];
    assert(posSize > 0);
    std::string posStr =
        m_currentLine.substr(m_nonGTPositions[POS_CHROM_END] + 1, posSize - 1);
    char *endPtr = nullptr;
    ...
    if (endPtr != (posStr.c_str() + posStr.size())) {
      PICOVCF_THROW_ERROR(MalformedFile,
                          "Invalid position (cannot parse): " << posStr);
    }
    return result;
  }
  ...
  double getQuality() const {
    ...
    if (endPtr != (qualString.c_str() + qualString.size())) {
      PICOVCF_THROW_ERROR(
          MalformedFile,
          "Invalid quality number (cannot parse): " << qualString);
    }
    return result;
  }
  ...
  std::vector<std::string> getFormat() const {
    ...
    if (formatString.size() == 2) {
      if (formatString != FORMAT_GT) {
        PICOVCF_THROW_ERROR(MalformedFile,
                            "The first item in FORMAT _must_ be " << FORMAT_GT);
      }
      return {formatString};
    }
    std::vector<std::string> result;
  ...
  IndividualIteratorGT getIndividualIterator() const {
    if (!hasGenotypeData()) {
      PICOVCF_THROW_ERROR(
          ApiMisuse,
          "Cannot iterate individuals when there is no genotype data");
    }
    return IndividualIteratorGT(m_currentLine,
                                m_nonGTPositions[POS_FORMAT_END] + 1);
  }
    ...
    for (size_t i = 0; i < m_nonGTPositions.size(); i++) {
      ...
      if (position == std::string::npos) {
        if (i < REQUIRED_FIELDS) {
          PICOVCF_THROW_ERROR(MalformedFile,
                              "Invalid line (missing required fields) at "
                                  << m_currentLine.substr(0, 100));
        } else {
...
public:
  ...
  explicit VCFFile(const std::string &filename)
...
#if VCF_GZ_SUPPORT
      m_infile = std::unique_ptr<BufferedReader>(
          new ZBufferedReader(filename, COMPRESSED_BUFFER_SIZE));
#else
      PICOVCF_THROW_ERROR(
          ApiMisuse,
          "picovcf was not compiled with zlib support (VCF_GZ_SUPPORT)");
#endif
    } else {
      m_infile = std::unique_ptr<BufferedReader>(
          new BufferedReader(filename, UNCOMPRESSED_BUFFER_SIZE));
    }
    parseHeader();
    ...
    if (version.substr(0, 5) == SUPPORTED_PREFIX) {
      if (version.size() > 6 &&
          !(version[6] != '0' || version[6] == '1' || version[6] == '2')) {
        PICOVCF_THROW_ERROR(MalformedFile,
                            "Unsupported VCF version: " << version);
      }
    } else {
      PICOVCF_THROW_ERROR(MalformedFile,
                          "Unsupported VCF version: " << version);
  ...
  void nextVariant() {
    if (picovcf_peek_eof(m_infile.get())) {
      PICOVCF_THROW_ERROR(
          ApiMisuse, "Tried to move to next variant when there aren't any");
    }
   ...
   * position 100-999.
   */
  void getRotatedWindow(RangePair individualRange, RangePair genomeRange,
                        VCFRotatedWindow &result,
                        FileOffset posAfterLastVariant = {0, 0}) {
    const size_t totalIndividuals = this->numIndividuals();
    if (individualRange.second <= individualRange.first ||
        individualRange.first >= totalIndividuals) {
    ...
    const size_t numIndividuals =
        std::min<size_t>(totalIndividuals, individualRange.second) -
        individualRange.first;
    ...
    while (this->hasNextVariant()) {
      ...
      if (position >= genomeRange.first && position < genomeRange.second) {
        ...
        for (size_t i = 0; i < individualRange.first && iterator.hasNext();
             i++, iterator.next())
          ;

        for (size_t i = 0;
             i < (individualRange.second - individualRange.first) &&
             iterator.hasNext();
             i++, iterator.next()) {
          ...
          if (result.sampleToMutation.empty()) {
            ...
            result.sampleToMutation.resize(numIndividuals *
                                           (result.isDiploid ? 2 : 1));
          }
          const size_t multiplier = (result.isDiploid ? 2 : 1);
          if (isMutation(allele1)) {
            result.sampleToMutation.at(multiplier * i)
                .emplace(currentVariantIndex, allele1);
          }
          // TODO should probably also check for consistent phasing.
          if ((allele2 != NOT_DIPLOID) != result.isDiploid) {
            PICOVCF_THROW_ERROR(ApiMisuse,
                                "getRotatedWindow requires consistent ploidy");
          }
          if (isMutation(allele2)) {
            result.sampleToMutation.at((multiplier * i) + 1)
                .emplace(currentVariantIndex, allele2);
          }
        }
        const VCFVariantInfo variantParsed = variant.parseToVariantInfo();
        result.parsedVariants.push_back(std::move(variantParsed));
      } else if (position >= genomeRange.second ||
                 !result.parsedVariants.empty()) {
        break;
      }
    }
...
private:
  void parseHeader() {
      ...
      if (lineBuffer.size() >= 1 && lineBuffer[0] == '#') {
        if (lineBuffer.size() >= 2 && lineBuffer[1] == '#') {
          ...
          if (key.empty() || value.empty()) {
            PICOVCF_THROW_ERROR(MalformedFile, "Invalid key/value pair at "
                                                   << m_infile->tell().first);
          }
          ...
          PICOVCF_ASSERT_OR_MALFORMED(headerColumns.size() >= 8,
                                      "Header line too short");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[0] == "#CHROM",
              "CHROM missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[1] == "POS",
              "POS missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(headerColumns[2] == "ID",
                                      "ID missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[3] == "REF",
              "REF missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[4] == "ALT",
              "ALT missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[5] == "QUAL",
              "QUAL missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[6] == "FILTER",
              "FILTER missing/out of order in header line");
          PICOVCF_ASSERT_OR_MALFORMED(
              headerColumns[7] == "INFO",
              "INFO missing/out of order in header line");
          if (headerColumns.size() > 8) {
            PICOVCF_ASSERT_OR_MALFORMED(
                headerColumns[8] == "FORMAT",
    ...
    PICOVCF_ASSERT_OR_MALFORMED(sawHeader,
                                "No header line (column names) found.");
  }
...
static inline size_t readStringLen(const uint64_t version,
...
static inline std::string readString(const uint64_t version,
                                     std::istream &inStream) {
...
template <typename T>
inline void samplesForBV(const T *buffer, SampleT index,
...
inline IGDSampleList getSamplesWithAlt(const uint8_t *buffer,
  ...
  IGDSampleList result;
  for (SampleT idx = 0; idx < picovcf_div_ceiling<SampleT, 8>(numSamples);
       idx++) {
...
public:
  ...
  IGDAllele(const std::string &stringVal, size_t numLongAlleles) : m_value() {
    if (stringVal.size() <= sizeof(m_value)) {
      for (size_t i = 0; i < stringVal.size(); i++) {
        ((char *)&m_value)[i] = stringVal[i];
      }
      PICOVCF_ASSERT_OR_MALFORMED(!(m_value & IS_LONG),
                                  "Invalid allele string " << stringVal);
      assert(stringVal.size() == sizeof(m_value) ||
             ((char *)&m_value)[sizeof(m_value) - 1] == 0);
    } else {
      PICOVCF_ASSERT_OR_MALFORMED(
          !(numLongAlleles & IS_LONG ||
            numLongAlleles > std::numeric_limits<uint32_t>::max()),
...
#pragma pack(push, 1)
  struct FixedHeader {
    ...
    uint64_t
        filePosIndex; // Byte offset in the file where the variant rows start.
...
#pragma pack(pop)
  ...
  explicit IGDData(const std::string &filename)
      : m_infile(filename, std::ios::binary),
        m_header({0, 0, 0, 0, 0, 0, 0, std::numeric_limits<uint64_t>::max(),
  ...
  IGDSampleList getSamplesWithAlt(VariantT variantIndex) {
    ...
    if (isSparse) {
      ...
      if (buffer) {
        ...
        PICOVCF_GOOD_OR_API_MISUSE(m_infile);
        return ::picovcf::getSamplesWithAlt((const uint8_t *)buffer.get(),
                                            numSamples());
      }
    }
    return {};
  ...
  std::vector<std::string> getIndividualIds() {
    ...
    if (0 != m_header.filePosIndividualIds) {
      ...
      PICOVCF_ASSERT_OR_MALFORMED(numLabels == m_header.numIndividuals,
                                  "Invalid number of individual ids");
  ...
  std::vector<std::string> getVariantIds() {
    ...
    if (0 != m_header.filePosVariantIds) {
      ...
      const size_t numLabels = readScalar<uint64_t>(m_infile);
      PICOVCF_ASSERT_OR_MALFORMED(numLabels == m_header.numVariants,
                                  "Invalid number of variant ids");
      for (size_t i = 0; i < numLabels; i++) {
        result.emplace_back(std::move(readString(m_header.version, m_infile)));
      }
    }
...
private:
  ...
  void readAndCheckHeader() {
    ...
    static_assert(
        CURRENT_IGD_VERSION == 4,
        "Remove/check backwards compatibility if incrementing version");
    PICOVCF_ASSERT_OR_MALFORMED(
        m_header.version == CURRENT_IGD_VERSION || m_header.version == 3,
    ...
    PICOVCF_ASSERT_OR_MALFORMED(m_header.filePosVariants > m_beforeFirstVariant,
                                "Invalid variant info position "
                                    << m_header.filePosVariants);
    PICOVCF_ASSERT_OR_MALFORMED(
        m_header.ploidy <= MAX_PLOIDY,
        "Invalid ploidy " << m_header.ploidy << " is greater than maximum of "
                          << MAX_PLOIDY);
    PICOVCF_ASSERT_OR_MALFORMED(
        m_header.numIndividuals <= MAX_SAMPLES / m_header.ploidy,
...
static inline void writeString(const std::string &value,
...
public:
  ...
  IGDWriter(uint32_t ploidy, uint32_t numIndividuals, bool isPhased)
    ...
    PICOVCF_ASSERT_OR_MALFORMED(ploidy <= MAX_PLOIDY,
                                "Ploidy exceeded maximum supported");
  }
   ...
   */
  void writeHeader(std::ostream &outStream, const std::string &source,
                   const std::string &description) {
    outStream.write(reinterpret_cast<const char *>(&m_header),
                    sizeof(m_header));
  ...
  void writeVariantSamples(std::ostream &outStream,
    ...
    const bool isSparse =
        (sampleList.size() <= (numSamples / IGDData::DEFAULT_SPARSE_THRESHOLD));
    m_index.push_back(std::move(
        makeEntry(genomePosition, isSparse, isMissing, outStream.tellp())));
    if (isSparse) {
   ...
   * individual.
   */
  void writeIndividualIds(std::ostream &outStream,
   ...
   * variant.
   */
  void writeVariantIds(std::ostream &outStream,
                       const std::vector<std::string> &labels) {
    if (labels.empty()) {
...
private:
  ...
  IGDData::IndexEntry makeEntry(VariantT genomePosition, const bool isSparse,
    ...
    if ((IGDData::BP_POS_FLAGS_MASK & position) != 0) {
      PICOVCF_THROW_ERROR(
          ApiMisuse, "Given genome position is too large: " << genomePosition);
    }
    if (isSparse) {
...
inline void vcfToIGD(const std::string &vcfFilename,
                     ...
                     std::string description = "", bool verbose = false,
  ...
  PICOVCF_ASSERT_OR_MALFORMED(variant1.hasGenotypeData(),
                              "VCF file has no genotype data");

  IndividualIteratorGT firstIndividual = variant1.getIndividualIterator();
  PICOVCF_ASSERT_OR_MALFORMED(firstIndividual.hasNext(),
                              "VCF file has no genotype data");
  VariantT allele1 = 0;
  VariantT allele2 = 0;
  const bool isPhased =
      firstIndividual.getAlleles(allele1, allele2, /*moveNext=*/false);
  ...
  while (vcf.hasNextVariant()) {
    ...
    while (individualIt.hasNext()) {
      ...
      PICOVCF_ASSERT_OR_MALFORMED(isPhasedI == isPhased,
                                  "Cannot convert VCF with mixed phasedness");
      const uint64_t ploidyI = (allele2 == NOT_DIPLOID) ? 1 : 2;
      PICOVCF_ASSERT_OR_MALFORMED(ploidyI == ploidy,
                                  "Cannot convert VCF with mixed ploidy");
    ...
    for (size_t altIndex = 0; altIndex < altAlleles.size(); altIndex++) {
      writer.writeVariantSamples(outFile, variant.getPosition(),
                                 variant.getRefAllele(), altAlleles[altIndex],
                                 variantGtData[altIndex], false);
      if (emitVariantIds) {
    ...
    if (!missingData.empty()) {
      writer.writeVariantSamples(outFile, variant.getPosition(),
                                 variant.getRefAllele(), "", missingData, true);
      if (emitVariantIds) {
        variantIds.emplace_back(currentVariantId);
      }
    }
  ...
  if (verbose) {
    std::cout << "Wrote " << writer.m_totalCount << " total variants"
              << std::endl;
    std::cout << "Of which " << writer.m_sparseCount << " were written sparsely"
              << std::endl;
  }
}
